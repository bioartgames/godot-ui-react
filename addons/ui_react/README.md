...
- **Who:** UI authors and small teams who want **fast iteration** without **per-screen glue scripts** for patterns the addon covers—using the dock **Graph** tab as the **primary** editor workbench when they want a **single orchestration view** (animation-tree-style), or **Inspector-only** when they prefer classic resource editing (**both** write the **same** `*.tscn` / `*.tres` shapes).
...
- **Blessed path (typical full screen):** attach **`UiReact*`** → assign **`*_state`** → optional **`animation_targets`** → optional **`wire_rules`** on **[`WIRING_LAYER.md`](docs/WIRING_LAYER.md)** §5 hosts (**`UiReactWireRuleHelper`** registers from each **`wire_rules`** export; author in the dock **Graph** tab **or** Inspector—**same** resources, **CB-035** / **P5.2** / **CB-058** ) → optional **`UiComputed*`** on bindings → optional **`UiTransactionalGroup`** + Apply/Cancel on **`UiReactButton`** / **`UiReactTextureButton`** (**`transactional_host`**, shared **`UiTransactionalScreenConfig`** on the binding) → optional **`action_targets`**. Official **examples** (below) mix these layers and stay **Inspector-authored** for **objective** proof.
...
| Runtime Output trace (**debug**, Graph tab footer opt-in **`CB-018C`** v1) | `UiReactRuntimeConsoleDebug` | `scripts/runtime/ui_react_runtime_console_debug.gd` |
...
**Persisted per project:** diagnostics/dock preferences (scan mode, **Group** mode, severity filters, output folder), ignored unused file paths (**`ui_react/settings/diagnostics/ignored_unused_state_paths`**), graph legend default, and Ui React shortcut settings are saved in **Project Settings** as internal keys. Session/layout restore state (last dock tab, Graph return node, graph split offset, scope preset payload) is stored in plugin layout metadata.

**When diagnostics update:** the list updates when you press **Rescan**, when you open or **switch the active edited scene** tab, when **EditorFileSystem** signals filesystem changes, and—in **Scan: Selection** mode—when the editor selection changes.
...
| Control | Purpose |
|--------|---------|
| **Scan** | **Selection** — selected nodes and their subtree `UiReact*` controls; diagnostics **rescan when editor selection changes**. **Entire scene** — all `UiReact*` nodes under the edited scene root. |
...
| `ui_react/settings/shortcuts/open_wiring_json` | `Alt+2` | Open/focus the **Graph** tab (opens bottom panel first when needed). |
