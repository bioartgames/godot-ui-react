...
static func reset_internal_state_for_tests() -> void:
	...
	_LiveGraph.reset_capture_registration_for_tests()
...
static func _wire_computed(computed: UiState, consumer: Node, _site_binding: StringName) -> void:
	...
	entry.consumer_weak = weakref(consumer)
...
static func _trigger_recompute_safe(computed: UiState) -> void:
	...
	var corr: Node = null
	if _wired.has(cid):
		var we: WiredEntry = _wired[cid]
		if we.consumer_weak != null:
			var n: Variant = we.consumer_weak.get_ref()
			if n is Node:
				corr = n as Node
	_LiveGraph.maybe_cmp(computed, corr)
