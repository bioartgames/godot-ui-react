...
const _LiveGraphDbgPlugin := preload(
	"res://addons/ui_react/editor_plugin/services/ui_react_live_graph_editor_debugger_plugin.gd"
)

var _dock: Control
var _live_graph_dbg: UiReactLiveGraphEditorDebuggerPlugin = null
...
func _enter_tree() -> void:
	...
	if dock_scene == null:
		...
		return
	_live_graph_dbg = _LiveGraphDbgPlugin.new()
	add_debugger_plugin(_live_graph_dbg)
	_dock = dock_scene.instantiate() as Control
	_dock.setup(self)
	_dock.set_plugin_owner(self)
	if _live_graph_dbg != null and _dock.has_method(&"notify_live_graph_debug_capture"):
		_live_graph_dbg.set_sink(Callable(_dock, &"notify_live_graph_debug_capture"))
	...
	# Restore/cold-start: edited scene sometimes attaches before this signal is wired, or shortly after deferred startup refresh.
	call_deferred(&"_defer_edited_scene_catchup_pass1")


func _defer_edited_scene_catchup_pass1() -> void:
	if _dock != null and _dock.has_method(&"notify_edited_scene_changed"):
		_dock.notify_edited_scene_changed()
	call_deferred(&"_defer_edited_scene_catchup_pass2")


func _defer_edited_scene_catchup_pass2() -> void:
	if _dock != null and _dock.has_method(&"notify_edited_scene_changed"):
		_dock.notify_edited_scene_changed()
...
func _exit_tree() -> void:
	...
	if _dock:
		...
		_dock.queue_free()
		_dock = null
	if _live_graph_dbg != null:
		remove_debugger_plugin(_live_graph_dbg)
		_live_graph_dbg = null
...
func _register_project_settings_property_info_v2() -> void:
	...
	for key in [
		...
		UiReactDockConfig.KEY_RUNTIME_CONSOLE_DEBUG_ENABLED,
		UiReactDockConfig.KEY_RUNTIME_GRAPH_LIVE_PULSES_ENABLED,
...
func _input(event: InputEvent) -> void:
	...
	if wiring_hit:
		_open_and_focus_tab(UiReactDock.TAB_GRAPH)
		get_viewport().set_input_as_handled()
...
func _open_and_focus_tab(tab_idx: int) -> void:
	...
	elif tab_idx == UiReactDock.TAB_GRAPH and _dock.has_method(&"open_and_focus_wiring"):
