...
static func _try_commit_sources_index(
	...
	if c0 == null:
		push_warning(
			"Ui React: could not find the computed resource for %s. Rescan the dependency graph or reopen the scene."
			% computed_context
		)
		return false
	...
	if head.is_empty() or not bool(head.get(&"ok", false)):
		push_warning(
			"Ui React: internal graph path %s is not valid for this edit. Click Rescan on the Graph tab and try again."
			% computed_context
		)
		return false
