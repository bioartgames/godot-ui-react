...
## Default when [constant KEY_SCAN_MODE] has never been set (no persisted user choice). Migrated / saved values win.
const DEF_SCAN_MODE := SCAN_MODE_SCENE
...
## Schema 4: removed togglable auto-refresh — Selection scan mode always rescans on editor selection change.
const _LEGACY_KEY_DIAGNOSTICS_AUTO_REFRESH := "ui_react/settings/diagnostics/auto_refresh"
...
const KEY_EDITOR_PLAY_MODE := "ui_react/settings/runtime/editor_play_mode"

const EDITOR_PLAY_MODE_OFF := 0
const EDITOR_PLAY_MODE_OUTPUT := 1
const EDITOR_PLAY_MODE_GRAPH := 2
const EDITOR_PLAY_MODE_GRAPH_FOLLOW := 3
const EDITOR_PLAY_MODE_OUTPUT_GRAPH := 4
const EDITOR_PLAY_MODE_OUTPUT_GRAPH_FOLLOW := 5
...
const DEF_RUNTIME_GRAPH_LIVE_PULSES_ENABLED := false
const DEF_EDITOR_PLAY_MODE := EDITOR_PLAY_MODE_OFF
...
static func migrate_project_settings_to_v2_clean_break() -> void:
	...
	if from_schema < 4 and ProjectSettings.has_setting(_LEGACY_KEY_DIAGNOSTICS_AUTO_REFRESH):
		ProjectSettings.set_setting(_LEGACY_KEY_DIAGNOSTICS_AUTO_REFRESH, null)
		changed = true
...
static func register_default_project_settings() -> void:
	...
	if ProjectSettings.has_setting(_LEGACY_KEY_DIAGNOSTICS_AUTO_REFRESH):
		ProjectSettings.set_setting(_LEGACY_KEY_DIAGNOSTICS_AUTO_REFRESH, null)
		added_defaults = true
	...
	if not ProjectSettings.has_setting(KEY_SCAN_MODE):
		ProjectSettings.set_setting(KEY_SCAN_MODE, DEF_SCAN_MODE)
		added_defaults = true
	...
	if not ProjectSettings.has_setting(KEY_RUNTIME_CONSOLE_DEBUG_ENABLED):
		...
		added_defaults = true
	if not ProjectSettings.has_setting(KEY_RUNTIME_GRAPH_LIVE_PULSES_ENABLED):
		ProjectSettings.set_setting(KEY_RUNTIME_GRAPH_LIVE_PULSES_ENABLED, DEF_RUNTIME_GRAPH_LIVE_PULSES_ENABLED)
		added_defaults = true
	...
	migrate_editor_play_mode_from_legacy_bools()


static func clamp_editor_play_mode(mode: int) -> int:
	if mode < EDITOR_PLAY_MODE_OFF or mode > EDITOR_PLAY_MODE_OUTPUT_GRAPH_FOLLOW:
		return EDITOR_PLAY_MODE_OFF
	return mode


static func editor_play_mode_includes_follow(mode: int) -> bool:
	return mode == EDITOR_PLAY_MODE_GRAPH_FOLLOW or mode == EDITOR_PLAY_MODE_OUTPUT_GRAPH_FOLLOW


static func legacy_runtime_bools_from_editor_play_mode(mode: int) -> Dictionary:
	mode = clamp_editor_play_mode(mode)
	var trace := (
		mode
		in [
			EDITOR_PLAY_MODE_OUTPUT,
			EDITOR_PLAY_MODE_OUTPUT_GRAPH,
			EDITOR_PLAY_MODE_OUTPUT_GRAPH_FOLLOW,
		]
	)
	var pulses := (
		mode
		in [
			EDITOR_PLAY_MODE_GRAPH,
			EDITOR_PLAY_MODE_GRAPH_FOLLOW,
			EDITOR_PLAY_MODE_OUTPUT_GRAPH,
			EDITOR_PLAY_MODE_OUTPUT_GRAPH_FOLLOW,
		]
	)
	return {&"trace": trace, &"pulses": pulses}


static func sync_runtime_keys_from_editor_play_mode(mode: int) -> void:
	mode = clamp_editor_play_mode(mode)
	var b := legacy_runtime_bools_from_editor_play_mode(mode)
	ProjectSettings.set_setting(KEY_RUNTIME_CONSOLE_DEBUG_ENABLED, bool(b.get(&"trace", false)))
	ProjectSettings.set_setting(KEY_RUNTIME_GRAPH_LIVE_PULSES_ENABLED, bool(b.get(&"pulses", false)))


static func save_editor_play_mode(mode: int) -> void:
	mode = clamp_editor_play_mode(mode)
	ProjectSettings.set_setting(KEY_EDITOR_PLAY_MODE, mode)
	sync_runtime_keys_from_editor_play_mode(mode)
	var err_ps := ProjectSettings.save()
	if err_ps != OK:
		push_warning("Ui React: could not save editor play mode.")


## When [member KEY_EDITOR_PLAY_MODE] was never written, derive it from legacy bool keys once and normalize booleans.
static func migrate_editor_play_mode_from_legacy_bools() -> void:
	if ProjectSettings.has_setting(KEY_EDITOR_PLAY_MODE):
		return
	var trace := bool(
		ProjectSettings.get_setting(KEY_RUNTIME_CONSOLE_DEBUG_ENABLED, DEF_RUNTIME_CONSOLE_DEBUG_ENABLED)
	)
	var pulses := bool(
		ProjectSettings.get_setting(KEY_RUNTIME_GRAPH_LIVE_PULSES_ENABLED, DEF_RUNTIME_GRAPH_LIVE_PULSES_ENABLED)
	)
	var mode := EDITOR_PLAY_MODE_OFF
	if trace and pulses:
		mode = EDITOR_PLAY_MODE_OUTPUT_GRAPH
	elif trace:
		mode = EDITOR_PLAY_MODE_OUTPUT
	elif pulses:
		mode = EDITOR_PLAY_MODE_GRAPH
	ProjectSettings.set_setting(KEY_EDITOR_PLAY_MODE, mode)
	sync_runtime_keys_from_editor_play_mode(mode)
	var err_ps := ProjectSettings.save()
	if err_ps != OK:
		push_warning("Ui React: could not migrate editor play mode settings.")
...
static func load_into(dock: UiReactDock) -> void:
	...
	var mode_id := int(ProjectSettings.get_setting(KEY_SCAN_MODE, DEF_SCAN_MODE))
	if mode_id != SCAN_MODE_SELECTION and mode_id != SCAN_MODE_SCENE:
		mode_id = DEF_SCAN_MODE
	if dock._mode_option:
		var idx := dock._mode_option.get_item_index(mode_id)
		dock._mode_option.select(idx if idx >= 0 else dock._mode_option.get_item_index(DEF_SCAN_MODE))
