...
const _MAX_COALESCED_REFRESH_PASSES := 16
## Attempts when [method EditorInterface.get_edited_scene_root] is still null after editor boot (each attempt awaits one [signal SceneTree.process_frame]).
const _STARTUP_NULL_SCENE_RETRY_COUNT := 48
## Stop scheduling null-root retries after this wall time from [method _run_startup_refresh] (avoids endless loops if no edited scene exists).
const _STARTUP_NULL_SCENE_MAX_WAIT_MSEC := 12000
...
## Armed only by [method _run_startup_refresh]; decremented each time [method refresh] runs with null edited scene root; cleared when root is present.
var _startup_scene_null_retries_remaining: int = 0
## Wall-clock start for null-root retry wave ([method _run_startup_refresh]); [code]0[/code] means no wave yet — do not apply max-wait until startup arms retries.
var _startup_null_scene_wave_started_msec: int = 0
## Coalesces [EditorUndoRedoManager] undo/redo into one deferred **Graph** tab refresh per frame.
...
func open_and_focus_wiring() -> void:
	if _tabs == null:
		return
	_tabs.current_tab = TAB_GRAPH
	_last_tab_for_persist = TAB_GRAPH
	call_deferred(&"_deferred_wiring_tab_activate")
...
func _dock_coalescer_flush() -> void:
	...
	_refresh_once_with_coalesce_drain()


func _refresh_once_with_coalesce_drain() -> void:
	var i := 0
	while true:
		i += 1
		refresh()
		if _refresh_coalescer == null:
			break
		if i >= _MAX_COALESCED_REFRESH_PASSES and _refresh_coalescer.has_burst_pending():
			call_deferred(&"_dock_coalescer_burst_overflow_followup")
			break
		if not _refresh_coalescer.take_burst_pending():
			break


func _dock_coalescer_burst_overflow_followup() -> void:
	if _refresh_coalescer != null and _refresh_coalescer.take_burst_pending():
		request_refresh(&"burst_overflow")
...
func _flush_undo_redo_graph_refresh() -> void:
	...
	if _tabs == null or _tabs.current_tab != TAB_GRAPH:
		return
...
func _run_startup_refresh() -> void:
	_startup_null_scene_wave_started_msec = Time.get_ticks_msec()
	_startup_scene_null_retries_remaining = _STARTUP_NULL_SCENE_RETRY_COUNT
	request_refresh(&"startup")


## Invoked via [method Object.call_deferred]; awaits one [signal SceneTree.process_frame] so [method EditorInterface.get_edited_scene_root] can attach (idle [code]call_deferred[/code] alone still runs in the same engine frame as the rest of the queue).
func _deferred_startup_scene_retry_tick() -> void:
	await get_tree().process_frame
	if not is_inside_tree():
		return
	if _startup_null_scene_wave_started_msec != 0:
		var elapsed_tick := Time.get_ticks_msec() - _startup_null_scene_wave_started_msec
		if elapsed_tick > _STARTUP_NULL_SCENE_MAX_WAIT_MSEC:
			return
	request_refresh(&"startup_scene_retry_deferred")


func notify_live_graph_debug_capture(message: String, data: Variant) -> void:
	if _wiring_panel != null and _wiring_panel.has_method(&"ingest_live_graph_debug_message"):
		_wiring_panel.call(&"ingest_live_graph_debug_message", message, data)


## Called from [EditorPlugin] when the edited scene tab changes (open/switch/empty).
func notify_edited_scene_changed() -> void:
	if _tabs != null and _tabs.current_tab == TAB_GRAPH:
		call_deferred(&"_deferred_wiring_tab_activate")
...
func _build_ui() -> void:
	...
	_mode_option.tooltip_text = (
		"Scan mode: Selection rescans diagnostics when editor selection changes (subtree scope); "
		+ "Entire scene scans all UiReact* nodes in the edited scene."
	)
...
func _connect_editor_signals() -> void:
	...
	if fs != null:
		...
		_editor_signal_lifecycle.scope.connect_bound(fs.resources_reload, _on_editor_filesystem_resources_reload)
		_editor_signal_lifecycle.scope.connect_bound(fs.resources_reimported, _on_editor_filesystem_resources_reimported)
		_editor_signal_lifecycle.scope.connect_bound(fs.script_classes_updated, _on_editor_filesystem_script_classes_updated)
		_editor_signal_lifecycle.scope.connect_bound(fs.sources_changed, _on_editor_filesystem_sources_changed)
...
func _on_selection_changed() -> void:
	if _wiring_panel != null and _wiring_panel.has_method(&"refresh"):
		_wiring_panel.call(&"refresh")
	if _mode_option.get_item_id(_mode_option.selected) == UiReactDockConfig.SCAN_MODE_SELECTION:
...
func _on_tabs_tab_selected(tab_idx: int) -> void:
	...
	if not _suppress_pref_save:
		if prev == TAB_GRAPH and tab_idx != TAB_GRAPH:
			if _wiring_panel != null and _wiring_panel.has_method(&"capture_session_for_persist"):
				_wiring_panel.call(&"capture_session_for_persist")
		UiReactDockConfig.save_last_tab_session(tab_idx)
	_last_tab_for_persist = tab_idx
	if tab_idx == TAB_GRAPH and not _suppress_pref_save:
		call_deferred(&"_deferred_wiring_tab_activate")
...
func _on_editor_filesystem_resources_reload(_resources: PackedStringArray) -> void:
	request_refresh(&"filesystem_resources_reload")


func _on_editor_filesystem_resources_reimported(_resources: PackedStringArray) -> void:
	request_refresh(&"filesystem_resources_reimported")


func _on_editor_filesystem_script_classes_updated() -> void:
	request_refresh(&"filesystem_script_classes_updated")


func _on_editor_filesystem_sources_changed(_exist: bool) -> void:
	request_refresh(&"filesystem_sources_changed")
...
func refresh() -> void:
	...
	if root == null:
		...
		_apply_filters()
		var elapsed_ms := (
			Time.get_ticks_msec() - _startup_null_scene_wave_started_msec
			if _startup_null_scene_wave_started_msec != 0
			else 0
		)
		if (
			_startup_null_scene_wave_started_msec != 0
			and elapsed_ms > _STARTUP_NULL_SCENE_MAX_WAIT_MSEC
		):
			_startup_scene_null_retries_remaining = 0
			return
		if _startup_scene_null_retries_remaining > 0:
			_startup_scene_null_retries_remaining -= 1
			call_deferred(&"_deferred_startup_scene_retry_tick")
		return
